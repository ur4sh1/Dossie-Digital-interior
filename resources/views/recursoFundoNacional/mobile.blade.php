    ...
    <div id="carouselExampleIndicatorsRecursoFundoNacional" class="carousel slide" data-ride="carousel"  data-interval="0">
        ...
        <div class="carousel-inner" data-interval="0">
            <div class="carousel-item active" data-interval="0">
                <div class="text-center" style="color: #1b1e21; background:lavenderblush"><b>CUSTEIO-{{ $data-3 }}</b></div>
                @foreach($itemRecursoFundoNacional as $i)
                    @foreach($m->recursoFundoNacional as $mfnm)
                        @if($mfnm->ano->ano == ($data-3) and $mfnm->item_recurso_fundo_nacional_id == $i->id and $mfnm->tipo_recurso_nacional_id == 1)
                            @if($mfnm->item_recurso_fundo_nacional_id == $i->id)
                                <div style="background:#b3e8ca" class="text-left descricaogrande">{{ $i->descricao }}</div>
                            @endif
                            <div class="text-right">{{ $mfnm->valor() }}</div>
...
<!--    INVESTIMENTO    -->
        ...
        <div id="carouselExampleIndicatorsRecursoFundoNacional2" class="carousel slide" data-ride="carousel"  data-interval="0">
            ...
            <div class="carousel-inner" data-interval="0">
                <div class="carousel-item active" data-interval="0">
                    <div class="text-center" style="color: #1b1e21; background:lavenderblush"><b>INVESTIMENTO-{{ $data-3 }}</b></div>
                    @foreach($itemRecursoFundoNacional as $i)
                        @foreach($m->recursoFundoNacional as $mfnm)
                            @if($mfnm->ano->ano == ($data-3) and $mfnm->item_recurso_fundo_nacional_id == $i->id and $mfnm->tipo_recurso_nacional_id == 2)
                                @if($mfnm->item_recurso_fundo_nacional_id == $i->id)
                                    <div style="background:#b3e8ca" class="text-left descricaogrande">{{ $i->descricao }}</div>
                                @endif
                                <div class="text-right">{{ $mfnm->valor() }}</div>
